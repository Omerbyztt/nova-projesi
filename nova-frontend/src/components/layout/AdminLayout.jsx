      ...
      <aside className="admin-sidebar">
        <div className="sidebar-brand" onClick={() => navigate(isAdmin ? '/dashboard' : '/home')} style={{cursor: 'pointer'}}>
          <div className="brand-logo">
            {currentUser?.department?.company?.logoUrl ? (
              <img 
                src={currentUser.department.company.logoUrl.startsWith('http') ? currentUser.department.company.logoUrl : `http://localhost:8080/uploads/logos/${currentUser.department.company.logoUrl}`} 
                alt="Company Logo" 
                style={{ width: '32px', height: '32px', borderRadius: '6px', objectFit: 'contain', backgroundColor: 'rgba(255, 255, 255, 0.1)', padding: '4px' }} 
              />
            ) : (
